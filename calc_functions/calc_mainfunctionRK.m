...
% full simulation
Ert=handle_NaNInf(myifft(const_GVD.*myfft(abs(Ert).*exp(1i.*pulse.w0.*mesh.t.*ones(mesh.rlength,1)),mesh),mesh))+(calc_mainFctOptimizeTime(mesh,medium,pulse,Ert))+(const.c/(2.*medium.n0)).*do_2Dfinitedifference(mesh,medium,cumsum(Ert.*mesh.dt,2),M_fd);
