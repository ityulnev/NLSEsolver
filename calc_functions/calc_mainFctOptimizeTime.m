...
Ert=-(medium.n2/const.c).*gradient(abs(Ert).^2.*Ert,mesh.dt).*5+const_PLSM.*cumsum(calc_2DeDensityADK(Ert,mesh,medium,beam,pulse).*mesh.dt.*Ert,2)+handle_NaNInf(const_ION.*(gradient(calc_2DeDensityADK(Ert,mesh,medium,beam,pulse),mesh.dt)./(real(Ert).^2)).*Ert);
