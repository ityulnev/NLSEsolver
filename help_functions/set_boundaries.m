...
switch M_fd.boundary{1,1}
    ...
    case 'open'
        ...
        dE=handle_NaNInf(log(Erf(3:4,:)./Erf(2:3,:))./(1i*mesh.dr));
...
switch M_fd.boundary{1,2}
    case 'const'
        Erf(end,:)=Erf(end,:);
    case 'open'
        dkr=handle_NaNInf(log(Erf(end-2:end-1,:)./Erf(end-3:end-2,:))./(1i*mesh.dr));
...
if strcmp(globproperties.mode,'debug')
    subplot(4,2,[3 4])
    plot(mesh.t,[(angle(Erf(1:3,:)))]); xlim([-20e-15 20e-15])
    subplot(4,2,[5 6])
    plot(mesh.t,[(angle(Erf(end-2:end,:)))]); xlim([-20e-15 20e-15])
    pause(0.1);
end
