...
function [r_smoothfct,f_smoothfct]=get_smoothfunction(mesh,pulse,E,type)
%find max
...
switch type
    case 'supergaussian'
    r_smoothfct=calc_supergaussian(mesh.r,mesh.R*2,10,0);
    r_smoothfct=handle_NaNInf(r_smoothfct');
    f_smoothfct=handle_NaNInf(calc_supergaussian(mesh.t,pulse.t_Ie2*mesh.Gfilter_fac,10,mesh.dt*(LRbounds(1,2)-mesh.indexfmid)));%mesh.dt*(LRbounds(1,2)-mesh.indexfmid))
end
